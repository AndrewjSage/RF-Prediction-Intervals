...
ui <- dashboardPage(
  dashboardHeader(title="LM and RF Prediction Intervals"),
  dashboardSidebar(sidebarMenu(
    menuItem("Simulation", tabName = "Simulation", icon = icon("dashboard")),
    menuItem("Real Data Analysis", tabName = "Real_Data", icon = icon("dashboard"))
  )),
  dashboardBody(
    tabItems(
      tabItem(tabName = "Simulation",
                ...
                column(2, 
                       selectInput("data", h5("Display"), 
                                   choices = list("Training Data" = "Train", 
                                                  "Test Data" = "Test"),
                                   selected = "Train"), 
                       sliderInput("level_sim", "Desired Coverage Level:",
                                   min = 0.7, max = 0.95, step=0.05,
                                   value = 0.9),
                       sliderInput("range_sim", "Range:",
            ...
            # third column  
              column(4,
                       checkboxGroupInput(inputId = "Estimate", label = "Display", 
                                          choices = list("True Expected Response" = "True",
                                                         "Linear Model (LM) Estimate" = "LMest", 
                                                         "Random Forest (RF) Estimate" = "RFest" 
                                          ),
                                          selected = "1"), #close checkbox
                       checkboxGroupInput(inputId = "Assumptions", label = "Prediction Interval Assumptions", 
      ...
      tabItem(tabName = "Real_Data",
              ...
              # second row
               fluidRow(
                 checkboxGroupInput("Exp_Vars", " Explanatory Variables to Include in Model:",character(0), inline=TRUE)
              ), #end fluid row
...
server <- function(input, output, session) {
  ...
  SimPreds <- reactive({CalcPreds(Dataset=SimulationRes()[[1]], RF=SimulationRes()[[2]], level=input$level_sim)})
  Plot <- reactive({CreatePlot(Dataset=SimPreds(), data=input$data, Estimate=input$Estimate, Assumptions=input$Assumptions, range=input$range_sim)})
  output$plot <- renderPlot(Plot())
  #MSPE <- reactive(CalculateMSPE(Dataset=SimulationRes(), range=input$range))
  output$RMSPEtable <- renderTable(CalculateRMSPE(Dataset=SimPreds(), range=input$range_sim))
  output$PItable <- renderTable(EvaluatePI(Dataset=SimPreds(), range=input$range_sim))
  ...
  CreateIntervalPlot <- function(Test, Estimate, Assumptions, range){  
  ...
  PredPlot <- reactive({CreateIntervalPlot(Test=Preds(), Estimate=input$Estimate, Assumptions=input$Assumptions, range=input$range)})
